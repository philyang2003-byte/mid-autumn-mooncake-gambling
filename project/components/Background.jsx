// Background: moon, clouds, hanging lanterns, starfield
const Background = () => {
  // precompute small decorative elements
  ...
  return (
    <div style={bgStyles.root}>
      {/* starfield */}
      <svg style={bgStyles.stars} viewBox="0 0 100 100" preserveAspectRatio="none">
        {stars.map((s,i)=>(
          <circle key={i} cx={s.x} cy={s.y} r={s.s*0.06} fill="#F5E7C8" opacity={s.o}>
            <animate attributeName="opacity" values={`${s.o};${s.o*0.2};${s.o}`} dur={`${2+s.d}s`} repeatCount="indefinite"/>
          </circle>
        ))}
      </svg>

      {/* Moon */}
      <div style={bgStyles.moonHalo}/>
      <div style={bgStyles.moon}>
      ...
      {/* drifting clouds */}
      <svg style={bgStyles.cloud1} viewBox="0 0 300 80">
      ...
      <svg style={bgStyles.cloud2} viewBox="0 0 300 80">
        <path d="M10,50 Q30,20 70,35 Q100,10 140,30 Q180,15 220,35 Q260,25 290,50 Q260,65 220,55 Q180,70 140,55 Q100,70 70,60 Q40,70 10,50 Z"
      ...
      <Lantern style={{left:"14%", top:"14%"}} size={66} delay={.7}/>
      <Lantern style={{right:"8%", top:"6%"}} size={92} delay={.3}/>
      <Lantern style={{right:"18%", top:"18%"}} size={60} delay={1.1}/>
      <Lantern style={{left:"3%", bottom:"8%"}} size={72} delay={.5}/>
      <Lantern style={{right:"4%", bottom:"10%"}} size={78} delay={.9}/>
...
const Lantern = ({style, size=70, delay=0}) => {
  const s = size;
  return (
    <div style={{position:"absolute", width:s, ...style, pointerEvents:"none"}}>
      {/* string from top */}
      <div style={{
      ...
      <div style={{
        ...
        <svg viewBox="0 0 100 130" width={s} height={s*1.3} style={{filter:`drop-shadow(0 0 ${s*0.25}px rgba(232,130,58,.45))`}}>
          <defs>
            <radialGradient id={`lg${s}${delay}`} cx="50%" cy="45%">
          ...
          {/* body */}
          <ellipse cx="50" cy="55" rx="40" ry="42" fill={`url(#lg${s}${delay})`}/>
          {/* ribs */}
