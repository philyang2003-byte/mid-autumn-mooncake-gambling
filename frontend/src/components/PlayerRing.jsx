...
export default function PlayerRing({ players, currentPlayerId, radiusX = 440, radiusY = 360, centerX, centerY }) {
  ...
  return (
    <>
      {players.map((p, i) => {
        ...
        return <PlayerCard key={p.id} player={p} active={p.id === currentPlayerId} x={x} y={y} />;
      })}
    </>
  );
}

function PlayerCard({ player, active, x, y }) {
  const totalWins = Object.values(player.prizes || {}).reduce((s, v) => s + v, 0);
  const winEntries = Object.entries(player.prizes || {}).filter(([, v]) => v > 0);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: active ? "1.5px solid #E8C26A" : "1px solid rgba(212,168,75,.35)",
        boxShadow: active
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{
            ...
            fontFamily: "Ma Shan Zheng, serif", fontSize: 22,
            color: active ? "#5A0F12" : "#E8C26A", flexShrink: 0,
          }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div style={{
              fontSize: 11, color: active ? "rgba(255,246,220,.75)" : "rgba(245,231,200,.5)",
              fontFamily: "Noto Serif SC, serif", marginTop: 2,
            }}>{player.age} 岁</div>
          </div>
        </div>
        ...
        {winEntries.length > 0 && (
          <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 3 }}>
            {winEntries.slice(0, 4).map(([key, cnt]) => (
              <span key={key} style={{
                fontFamily: "Ma Shan Zheng, serif", fontSize: 11,
                padding: "1px 5px", borderRadius: 3,
                background: "rgba(232,194,106,.2)", border: "1px solid rgba(232,194,106,.5)",
                color: "#FFE9A8",
              }}>{PRIZE_NAMES[key]} ×{cnt}</span>
            ))}
            {winEntries.length > 4 && (
              <span style={{ fontSize: 10, color: "rgba(255,246,220,.6)", alignSelf: "center" }}>+{winEntries.length - 4}</span>
            )}
          </div>
        )}
        ...
        {active && (
          <div style={{
            ...
            color: "#5A0F12", fontFamily: "Ma Shan Zheng, serif", fontSize: 13,
            padding: "2px 10px", borderRadius: 10,
            whiteSpace: "nowrap", boxShadow: "0 2px 8px rgba(0,0,0,.5)",
